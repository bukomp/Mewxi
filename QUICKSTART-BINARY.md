# claude-usage — Quickstart (binary only)

You received the `claude-usage` binary. Here's how to install and use it. No Rust toolchain needed.

## 1. Install

```sh
mkdir -p ~/.local/bin
mv claude-usage ~/.local/bin/
...
```
       ...
       └───────┬────────────────┘
               ...
               ▼
       ┌────────────────────────┐      fetches       ┌──────────────────────┐
       │   claude-usage watch   │ ─────────────────▶ │  api.anthropic.com   │
       │  parse · fetch · render│ ◀───────────────── │    /oauth/usage      │
       └───────┬────────────────┘      live data     └──────────────────────┘
               │ writes
               ▼
       ┌────────────────────────┐
...
4. Still stale? Run `claude-usage status` manually — if that works, the watcher is the problem; if it errors, your OAuth token probably isn't accessible. Set it explicitly:

   ```sh
   export CLAUDE_USAGE_OAUTH_TOKEN="<your token>"
   ```
