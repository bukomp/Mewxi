...
async fn handle(req: Value, no_live: bool) -> Option<Value> {
    ...
    match method {
        "initialize" => Some(ok(id, json!({
            ...
            "capabilities": { "tools": {} },
            "serverInfo": { "name": "mewxi", "version": env!("CARGO_PKG_VERSION") }
        }))),
...
fn tool_defs() -> Value {
    json!([
        {
            "name": "list_accounts",
            "description": "List every Claude Code account mewxi knows about (one per CLAUDE_CONFIG_DIR), with its directory and configured default.",
