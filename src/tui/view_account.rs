...
fn render_wide(f: &mut Frame, area: Rect, pa: &PerAccount, m: &Metrics) {
    ...
    widgets::render_footer(f, rows[4], "3", "Tab next account · Esc back · live via api.anthropic.com/api/oauth/usage", true);
}
...
fn render_narrow(f: &mut Frame, area: Rect, pa: &PerAccount, m: &Metrics) {
    ...
    for (rect, key) in rects.iter().zip(keys.iter()) {
        match *key {
            ...
            "footer" => widgets::render_footer(f, *rect, "3", "Tab next · Esc back", true),
            _ => {}
        }
    }
}
